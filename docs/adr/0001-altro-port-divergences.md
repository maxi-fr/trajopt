...
*(Note: Superseded in part by ADR 0004: `options.multiplier_projection` default was changed from `True` to `False` to eliminate redundant Gram matrix assembly and dense solve overhead, matching shipped Julia behavior).*
...
`options.multiplier_projection` (originally default `True`, now default `False` per ADR 0004).
...
*(Note: Superseded by ADR 0004: `altro_solve` now bypasses PN when disabled and wraps `pn_solve` in `jax.lax.cond(run_pn, ...)` inside the trace, avoiding dense KKT assembly and factorization whenever AL has already converged to tolerance).*
...
`inner_failed` handling). As recorded in ADR 0004, this has been refined to `jax.lax.cond` to avoid
