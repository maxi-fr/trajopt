...
qrefs = [rand(QuatRotation) for i = 1:RobotDynamics.num_rotations(s)]
...
quat2mrp(q) = Rotations.params(MRP(QuatRotation(q)))
mrp2quat(g) = Rotations.params(QuatRotation(MRP(g)))
rp2quat(p) = Rotations.params(QuatRotation(RodriguesParam(p)))
quat2rp(q) = Rotations.params(RodriguesParam(QuatRotation(q)))

Rotations.jacobian(QuatRotation, MRP(q)) ≈
...
Rotations.jacobian(QuatRotation, RodriguesParam(q)) ≈
...
@btime Rotations.jacobian(QuatRotation, RodriguesParam($q))
